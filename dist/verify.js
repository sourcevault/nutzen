...
(function(){
  var com, reg, z, R, verify, num, try_matching, map_fname;
  com = require("./common");
  ...
  map_fname = function(fname){
    ...
    return ['ok', shorthand, validator];
  };
  verify.main = function(path, args, data){
    var rets, ref$, _, patt, clean_args, ret, cont, fname, validator, errorType;
    switch (path.length) {
    case 0:
      if (data.fns.length === 0 && data.def === null) {
        ...
        default:
          ref$ = rets[0], _ = ref$[0], patt = ref$[1], clean_args = ref$[2];
          switch (patt) {
          case 'f':
            return ['ok', 'def', clean_args];
          case 'nf':
            return ['ok', 'ar', clean_args];
          case 'ff':
            return ['ok', 'wh', clean_args];
          case 'nff':
            return ['ok', 'arwh', clean_args];
          }
        }
      } else {
    ...
    case 1:
      ...
      cont = ret[0], fname = ret[1], validator = ret[2];
      switch (cont) {
      case 'fault':
        return ret;
      }
      ret = validator(args);
      cont = ret[0], errorType = ret[1], clean_args = ret[2];
      switch (cont) {
      case 'fault':
        return ['fault', 'input', fname, errorType];
      }
      switch (fname) {
      case 'def':
        switch (data.ldef) {
